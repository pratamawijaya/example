...
public class GithubNetworkService extends Service {
  ...
  public interface GithubClient {
    @GET("/gists") Observable<List<Gist>> gists();

    @GET("/gists/{id}") Observable<GistDetail> gist(@Path("id") String id);
  }
  ...
  @Override public void onCreate() {
    ...
    if (mGithubClient == null) {
      mGithubClient = new RestAdapter.Builder().setRequestInterceptor(new RequestInterceptor() {
  ...
  public void getGist() {
    mGithubClient.gists()
        .flatMap(new Func1<List<Gist>, Observable<Gist>>() {
          @Override public Observable<Gist> call(List<Gist> gists) {
        ...
        .groupBy(new Func1<Gist, String>() {
          @Override public String call(Gist gist) {
            return gist.getId();
          }
        })
        .flatMap(new Func1<GroupedObservable<String, Gist>, Observable<GistDetail>>() {
          @Override public Observable<GistDetail> call(
              GroupedObservable<String, Gist> stringGistGroupedObservable) {
        ...
        .subscribe(new Action1<List<GistDetail>>() {
          @Override public void call(List<GistDetail> gistDetailList) {
